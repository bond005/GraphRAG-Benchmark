...
def load_graph_from_gml(gml_path: str) -> ig.Graph:
    """
    Load graph data from GML file.
...
def process_graphs_ragu(base_path: str, folder_name: str) -> List[Dict]:
    """
    Process graph data generated by RAGU.
    ...
    for subdir, dirs, files in os.walk(base_path):
        for file in files:
            if file == 'knowledge_graph.gml':
                gml_path = os.path.join(subdir, file)
                try:
                    g = load_graph_from_gml(gml_path)
                    result = analyze_graph(g)
                    results.append(result)
                except Exception as e:
                    print(f"Error processing {gml_path}: {e}")
...
def calculate_indexing_metrics(framework: str, base_path: str, folder_name: Optional[str] = None) -> Dict[str, float]:
    ...
    if framework == 'microsoft_graphrag':
        ...
        results = process_graphs_ragu(base_path, folder_name or "")
    else:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True,
        choices=['microsoft_graphrag', 'lightrag', 'fast_graphrag', 'hipporag2', 'graphml', 'ragu'],
